(() => {
  ...
  const modeCopy = {
    plain: {
      ...
      text: "Move through the suspended strings, then select a remodeling mode."
    },
    formation: {
      title: "Formation",
  ...
  const updatePoint = point => {
    ...
    const restorationStrength = state.mode === "formation"
      ? 0.027
      : state.mode === "resorption"
        ? 0.0085
        : 0.012;

    const restorationBoost = 0.55 + state.modeProgress * 0.45;
    const targetStrength = restorationStrength * restorationBoost;
  ...
  const drawStrings = () => {
    ...
    for (const column of state.columns) {
      ...
      const stringColor = state.mode === "resorption"
        ? palette.primary
        : palette.ink;

      const stringOpacity = state.mode === "plain"
        ? 0.2
  ...
  const drawPointerField = () => {
    ...
    gradient.addColorStop(
      0,
      rgba(
        state.mode === "resorption" ? palette.primary : palette.gold,
